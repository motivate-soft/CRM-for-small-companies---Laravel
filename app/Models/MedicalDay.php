...
class MedicalDay extends Model
{
    use CrudTrait;

    protected $table = 'medical_days';

    protected $fillable = ['status', 'date', 'employee_id', 'event_type_id', 'photo', 'comment', 'amount', 'leave_type', 'reject_message', 'company_is_read', 'employee_is_read'];
    ...
    protected static function boot()
    {
		...
		static::deleting(function($obj) {
            \Storage::disk('uploads')->delete($obj->photo);
        });
    }
    ...
    {
        $this->uploadFileToDisk($value, 'image', 'uploads', 'medical_day/' . backpack_user()->employee->id);
    }
    ...
    public function setPhotoAttribute($value)
    {
        ...
        $disk = "uploads";
        $destination_path = 'medical_day/' . $this->id;
		
		$this->uploadFileToDisk($value, $attribute_name, $disk, 'medical_day/' . $this->id);
