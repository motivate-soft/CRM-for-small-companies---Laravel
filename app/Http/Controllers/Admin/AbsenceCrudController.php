...
class AbsenceCrudController extends CrudController
{
    public function setup()
    {
        ...
        */
        ...
        $this->crud->setRoute(config('backpack.base.route_prefix') . '/absences');
        $this->crud->setEntityNameStrings(trans('fields.absence'), trans('fields.absences'));
        ...
        */
        ...
        if(backpack_user()->role == User::ROLE_COMPANY) {
            ...
            $this->crud->addColumn([
                ...
                'orderLogic' => function ($query, $column, $columnDirection) {
                    return $query->leftJoin('employees', 'employees.id', '=', 'absence_days.employee_id')
                        ->leftJoin('users', 'users.id', '=', 'employees.user_id')
                        ->orderBy('users.name', $columnDirection)->select('absence_days.*');
                }
            ])->afterColumn('status');
        ...
        // add asterisk for fields that are required in AbsenceRequest
        ...
        if(backpack_user()->role == User::ROLE_COMPANY) {
            ...
            foreach($employees as $employee){
                ...
                Absence::where('employee_id', $employee_id)->update(['company_is_read'=>1]);
            }
        }else if (backpack_user()->role == User::ROLE_EMPLOYEE) {
            $employee_id = backpack_user()->employee->id;
            Absence::where('employee_id', $employee_id)->update(['employee_is_read'=>1]);
        }
    }
    ...
    public function store(StoreRequest $request)
    {
        ...
        $notification = [
            'title' => "Biodactil",
            'sound' => true,
            'body' => trans('fields.notification_request_absence'),
    ...
    {
        $id = $request->id;
        $before_status = Absence::find($id)->status;
        ...
        if ($before_status != $after_status) {
            ...
            Absence::find($request->id)->update(['employee_is_read'=>0]);
            // Send notification from employee to company
            $notification = [
                'title' => "Biodactil",
                'sound' => true,
            ...
            if (isset($token)) {
                ...
                $notification_item->type = 'absence';
