...
class ExpenseCrudController extends CrudController
{
    public function setup()
    {
        ...
        */
        ...
        $this->crud->setEntityNameStrings(trans('fields.expenses'), trans('fields.expenses'));
        ...
        */
        ...
        if (backpack_user()->role == User::ROLE_EMPLOYEE) {
            $event_type = EventMandatoryType::where('type', 'expense')->where('company_id', backpack_user()->employee->company->id)->first();
        } else if (backpack_user()->role == User::ROLE_COMPANY) {
            $event_type = EventMandatoryType::where('type', 'expense')->where('company_id', backpack_user()->company->id)->first();
        }
        ...
        if(backpack_user()->role == User::ROLE_COMPANY) {
            ...
            $this->crud->addColumn([
                ...
                'orderLogic' => function ($query, $column, $columnDirection) {
                    return $query->leftJoin('employees', 'employees.id', '=', 'expenses.employee_id')
                        ->leftJoin('users', 'users.id', '=', 'employees.user_id')
                        ->orderBy('users.name', $columnDirection)->select('expenses.*');
                }
            ])->afterColumn('status');
...
//                    'end_name' => 'end_date', // the db column that holds the end_date
        ...
        $this->crud->setRequiredFields(ExpenseRequest::class, 'create');
        $this->crud->setRequiredFields(ExpenseRequest::class, 'edit');
        ...
        if(backpack_user()->role == User::ROLE_COMPANY) {
            ...
            foreach($employees as $employee){
                ...
                Expense::where('employee_id', $employee_id)->update(['company_is_read'=>1]);
            }
        }else if (backpack_user()->role == User::ROLE_EMPLOYEE) {
            $employee_id = backpack_user()->employee->id;
            Expense::where('employee_id', $employee_id)->update(['employee_is_read'=>1]);
        }
    }
    ...
    public function store(ExpenseRequest $request)
    {
        ...
        $notification = [
            'title' => "Biodactil",
            'sound' => true,
            'body' => trans('fields.notification_request_expense'),
    ...
    {
        $id = $request->id;
        $before_status = Expense::find($id)->status;
        ...
        if ($before_status != $after_status) {
            ...
            Expense::find($request->id)->update(['employee_is_read'=>0]);
            // Send notification from employee to company
            $notification = [
                'title' => "Biodactil",
                'sound' => true,
	...
	//            return $webNotification;
            ...
            if (isset($token)) {
                ...
                $notification_item->type = 'expense';
