...
<li filter-name="{{ $filter->name }}"
    ...
    <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">{{ date("Y") }} <span class="caret"></span></a>
    <ul class="dropdown-menu">
        <li><a parameter="{{ $filter->name }}" dropdownkey="All" href="">All</a></li>
        ...
        jQuery(document).ready(function($) {
            $("li.dropdown[filter-name={{ $filter->name }}] .dropdown-menu li a").click(function(e) {
                ...
                if (URI(new_url).hasQuery('{{ $filter->name }}', true)) {
                    ...
                    console.log(value);
                    $("li[filter-name={{ $filter->name }}] a[class=dropdown-toggle]").html(value+'<span class="caret"></span>');
                }
                else
                {
            ...
            $("li[filter-name={{ $filter->name }}]").on('filter:clear', function(e) {
                ...
                $("li[filter-name={{ $filter->name }}] .dropdown-menu li").removeClass('active');
                $("li[filter-name={{ $filter->name }}] a[class=dropdown-toggle]").html('2019 <span class="caret"></span>');
            });
        });
    </script>
@endpush
