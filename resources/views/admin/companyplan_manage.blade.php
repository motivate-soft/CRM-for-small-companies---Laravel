                ...
                <div class="box-body">
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <th>{{ trans('fields.plan') }}</th>
                            <th>{{ trans('fields.free_days') }}</th>
                        ...
                        @foreach ($companies as $item)
                            <tr>
                                ...
                                <td>
                                    @if ($item->currency)
                                        {{  $item->currency->short_key }}
                                    @else
                                    @endif
                                </td>
                                <td>
                                    @php
                                        $plan = $item->companyPlan;
                                        if ($plan) {
                                            $plan_type = explode('_', explode('-', $plan->company_plan_id)[1])[1];
                                            $plan_labels = \App\Models\Plan::getPlanLabelList();
                                            $labels = $plan_labels[$plan_type];
                                        }
                                    @endphp

                                    @if($plan && $plan->billing_status != \App\GlobalConstant::COMPANY_PLAN_STATUS_UNLIMITED)
                                    <span class="label
                                        @if($plan_type == 0)
                                            label-warning
                                        @elseif($plan_type == 1)
                                        ...
                                        @endif
                                            ">
                                        {{ \App\Models\Plan::getPlanLabelList()[$plan_type] }}
                                     </span>
                                    @endif
                                </td>
                                <td>
                                    @if($plan && $plan->billing_status == \App\GlobalConstant::COMPANY_PLAN_STATUS_FREE)
                                       {{ $plan->free_days }}
                                    @endif
                                </td>
                                <td>
                                    @if($plan)
                                        <span class="label
                                        @if($plan->billing_status == \App\GlobalConstant::COMPANY_PLAN_STATUS_FREE)
                                            label-warning
                                        @elseif($plan->billing_status == \App\GlobalConstant::COMPANY_PLAN_STATUS_APPROVED)
                                            label-primary
                                        @elseif($plan->billing_status == \App\GlobalConstant::COMPANY_PLAN_STATUS_UNLIMITED)
                                            label-success
                                        @elseif($plan->billing_status == \App\GlobalConstant::COMPANY_PLAN_STATUS_EXPIRED)
                                            label-info
                                        @elseif($plan->billing_status == \App\GlobalConstant::COMPANY_PLAN_STATUS_PENDING)
                                            label-danger
                                        @elseif($plan->billing_status == \App\GlobalConstant::COMPANY_PLAN_STATUS_REJECTED)
                                            label-default
                                        @endif
                                             ">
                                        {{ trans('fields.' . $plan->billing_status) }}
                                     </span>
                                    @endif
                                </td>
                                <td>
                                    <a class="btn btn-xs btn-default" href="{{ url('company_plans/edit/' . $item->id) }}" data-toggle="tooltip" ><i class="fa fa-edit"></i> {{ trans('backpack::crud.edit') }}</a>
                                    @if($plan)
                                    <a class="btn btn-xs btn-default delete_plan" data-id="{{ $item->id }}" href="#" data-toggle="tooltip" ><i class="fa fa-remove"></i> {{ trans('backpack::crud.delete') }}</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
...
@section('after_scripts')
    ...
    <script>
        ...
        $('.delete_plan').on('click', function () {
            var r = confirm("Do you want to delete plan for this company?");

            var id = $(this).data('id');
            if (r == true) {
                window.location.href = '{!! url('company_plans/delete/') !!}' + "/" + id;
            } else {
