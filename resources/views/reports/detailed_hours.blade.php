                                        ...
                                        <table class="table table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>{{ trans('fields.first_in') }}</th>
                                                    <th>{{ trans('fields.last_out') }}</th>
                                                    <th>{{ trans('fields.total_pauses') }}</th>
                                                    <th>{{ trans('fields.total_hours_in_schedule') }}</th>
                                                    <th>{{ trans('fields.total_hours_worked') }}</th>
                                                    <th>{{ trans('fields.hours_worked_in_schedule') }}</th>
                                                    <th>{{ trans('fields.hours_worked_outside_schedule') }}</th>
                                            ...
                                            @for($day = 1; $day <= Carbon\Carbon::createFromFormat('d/m/Y', '01/' . $filters['date'])->daysInMonth; $day++)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php
                                                            if(isset($items[$day])) {
                                                                foreach ($items[$day]->reverse()->values() as $time) {
                                                    ...
                                                    <td>
                                                        @php
                                                            if(isset($items[$day])) {
                                                                foreach ($items[$day]->values() as $time) {
                                                        ...
                                                        @php
                                                            if(isset($items[$day])) {
                                                                ...
                                                                for($i = 0; $i < $actions->count(); $i++){
                                                                    if(isset($actions[$i + 1])) {
                                                                        $seconds += (int)\Carbon\Carbon::parse($actions[$i]->datetime)->diffInSeconds(\Carbon\Carbon::parse($actions[++$i]->datetime));
                                                                    }
                                                                }
                                                    ...
                                                    <td>
                                                        @php
                                                            if(isset($items[$day])) {
                                                                $hours_worked_in_schedule = \App\Models\Action::getHoursWorkedInSchedule($items, $day, $schedule, $filters);
                                                                echo $hours_worked_in_schedule;
                                                            }
                                                        @endphp
                                                    </td>
                                                    <td>
                                                        @php
                                                            if(isset($items[$day])) {
                                                                echo \Carbon\Carbon::parse($actual_time)->diff(\Carbon\Carbon::parse($hours_worked_in_schedule))->format('%H:%I:%S');
                                                            }
                                                        @endphp
                                                    </td>
                                                    <td>
